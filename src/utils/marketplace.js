...
import approvalProgram from "!!raw-loader!../contracts/pokemon_contract_approval.teal";
...
export const createCardAction = async (senderAddress, card) => {
    console.log("Adding pokemon card...")
    ...
    let name = new TextEncoder().encode(card.name);
...
export const toggleSaleAction = async (senderAddress, card) => {
    console.log("Toggling sale...");
...
export const getCardAction = async () => {
    ...
    for (const transaction of transactionInfo.transactions) {
        ...
        if (appId) {
            ...
            let card = await getApplication(appId)
            if (card) {
                cards.push(card)
            }
        }
    }
    console.log("Pokemon cards fetched.")
    return cards
}
...
const getApplication = async (appId) => {
    try {
        ...
        if (getField("ATTRIBUTES", globalState) !== undefined) {
            let field = getField("ATTRIBUTES", globalState).value.bytes
            attributes = base64ToUTF8String(field)
        }
        ...
        if (getField("ISFORSALE", globalState) !== undefined) {
